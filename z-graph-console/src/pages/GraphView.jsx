...
export default function GraphView({ server }) {
  ...
  const fetchGraph = useCallback(async () => {
    ...
    setError(null);
    try {
      const nodeData = await api.query('MATCH (n) RETURN n', 'main');
      const nodes = (nodeData || []).map((row, i) => {
        const n = row.n || row;
      ...
      let edges = [];
      try {
        const edgeTypesData = await api.query('SHOW EDGES', 'main');
        const edgeTypes = Array.isArray(edgeTypesData) ? edgeTypesData : [];
        for (const et of edgeTypes) {
          const typeName = et.Name || et.name || '';
          if (!typeName) continue;
          try {
            const edgeData = await api.query(`MATCH (a)-[r:${typeName}]->(b) RETURN a, r, b`, 'main');
